...
Vote = Tuple[str, int]
PollWithOption = Tuple [int, str, str, str, int] # This is for the joint tables
PollResults = Tuple[int, str, int, float] # the float is for the %
...
INSERT_OPTION = "INSERT INTO options (option_text, poll_id) VALUES %s;"  # the argument %s is a tuple
...
def get_poll_and_vote_results(connection, poll_id: int) -> List[PollResults]:
    with connection:
        with connection.cursor() as cursor:
            ...
            return cursor.fetchall() # returns multiple rows: one percentage per option
...
def get_random_poll_vote(connection, option_id: int) -> Vote:
    with connection:
        with connection.cursor() as cursor:
            ...
            return cursor.fetchone() # There is just one row to return
...
#             cursor.execute("SELECT id FROM polls ORDER BY id DESC LIMIT 1;")  # get poll id
#
#             poll_id = cursor.fetchone()[0]   # save poll id in a variable. fetchone acts as a tuple
#             option_values = [(option_text, poll_id) for option_text in options]  # list of tuples
#
#             for option_value in option_values:
#                 cursor.execute(INSERT_OPTION, (option_value,))
#                  # option value is a tuple but I still need to pass argument as tuple (so a tuple of tuple)
...
def add_poll_vote(connection, username: str, option_id: str):
    with connection:
        with connection.cursor() as cursor:
